...
int main(int argc, char *argv[])
{
  ...
  int blk_x_static = blk_x;
  int blk_y_static = blk_y;
  ...
  if (my_cart_rank == 0)
  {
    for (int p=0; p<px*py; ++p)
    {
      ...
      MPI_Recv(&u_new_all[source_coords[0]*Nx*blk_y_static + source_coords[1]*blk_x_static], 1, blk_vector, p, p, comm_cart, status);
      printf("%d received!\n", p);
    }
  }
  ...
  for(int n=2; n<Nt; ++n) {
    ...
    if (next_coords[1]>=0 && next_coords[1]<px && next_coords[0]>=0 && next_coords[0]<py)
      MPI_Cart_rank(comm_cart, next_coords, &next_cart_rank);
    if (prev_coords[1]>=0 && prev_coords[1]<px && prev_coords[0]>=0 && prev_coords[0]<py)
  ...
  if (my_cart_rank == 0)
  {
    for (int p=0; p<px*py; ++p)
    {
      MPI_Probe(p, p, comm_cart, status);
      MPI_Cart_coords(comm_cart, p, ndims, source_coords);
      MPI_Recv(&u_new_all[source_coords[0]*(Nx*Ny)/(px*py)*px + source_coords[1]*Nx/px], 1, blk_vector, p, p, comm_cart, MPI_STATUS_IGNORE);
    }
  }
