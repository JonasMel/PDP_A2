...
int main(int argc, char *argv[])
{
  ...
  if ((my_coords[0] == 0 || my_coords[0] == px-1) && (my_coords[1] == 0 || my_coords[1] == py-1)) { // corner points
    blk_x++;
    blk_y++;
  } else if (my_coords[0] == 0 || my_coords[0] == px-1) { // edges columns
    blk_x++;
    blk_y += 2;
  } else if (my_coords[1] == 0 || my_coords[1] == py-1) { // edges rows
    blk_x += 2;
    blk_y++;
  } else { // inner points
    blk_x += 2;
  ...
  for(int i = 1; i < (blk_y-1); ++i) {
    for(int j = 1; j < (blk_x-1); ++j) {
      double x = (j+1)*blk_x*my_coords[0]*dx;
      double y = (i+1)*blk_y*my_coords[1]*dx;
  ...
  MPI_Type_vector(Ny/py, Nx/px, blk_x, MPI_DOUBLE, &blk_vector);
  MPI_Type_commit(&blk_vector);
  int offset;
  
  // Send blocks to root node
  if (my_coords[0] == 0 && my_coords[1] == 0) // upper left corner
    offset = 0;
  else if (my_coords[1] == 0) // upper edge row
    offset = 1;
  else if (my_coords[0] == 0) // left edge column
    offset = blk_x;
  ...
  if (my_cart_rank == 0)
  {
    for (int p=0; p<px*py; ++p)
    {
      ...
      MPI_Recv(&u_new_all[source_coords[1]*(Nx*Ny)/(px*py)*px + source_coords[0]*Nx/px], 1, blk_vector, p, p, comm_cart, status);
      printf("%d received!\n", p);
    }
  }
  ...
  for(int n=2; n<Nt; ++n) {
    ...
    if (next_coords[0]>=0 && next_coords[0]<px && next_coords[1]>=0 && next_coords[1]<py)
      MPI_Cart_rank(comm_cart, next_coords, &next_cart_rank);
    if (prev_coords[0]>=0 && prev_coords[0]<px && prev_coords[1]>=0 && prev_coords[1]<py)
      MPI_Cart_rank(comm_cart, prev_coords, &prev_cart_rank);
    
    if (my_coords[0] == 0) { // left edge nodes
      MPI_Send(&u[blk_x-2], 1, col_vector, next_cart_rank, next_cart_rank, comm_cart);
    } else if (my_coords[0] != 0 && my_coords[0] != px-1) { // inner nodes
      MPI_Sendrecv(&u[blk_x-2], 1, col_vector, next_cart_rank, next_cart_rank, &u[0], 1, col_vector, prev_cart_rank, my_cart_rank, comm_cart, MPI_STATUS_IGNORE);
    } else if (my_coords[0] == px-1) { // right edge nodes
    ...
    if (my_coords[1] == 0) { // top edge nodes
      MPI_Send(&u[blk_x-2], 1, MPI_DOUBLE, next_cart_rank, next_cart_rank, comm_cart);
    } else if (my_coords[0] != 0 && my_coords[0] != px-1) { // inner nodes
      MPI_Sendrecv(&u[blk_x-2], 1, col_vector, next_cart_rank, next_cart_rank, &u[0], 1, col_vector, prev_cart_rank, my_cart_rank, comm_cart, MPI_STATUS_IGNORE);
    } else if (my_coords[0] == px-1) { // botton edge nodes
      MPI_Recv(&u[0], 1, col_vector, prev_cart_rank, my_cart_rank, comm_cart, MPI_STATUS_IGNORE);
    }
  ...
  if (my_cart_rank != 0)
    MPI_Send(&u_new[offset], 1, blk_vector, 0, my_cart_rank, comm_cart);
  ...
  if (my_cart_rank == 0)
  {
    ...
    {
      MPI_Cart_coords(comm_cart, p, ndims, source_coords);
      MPI_Recv(&u_new_all[source_coords[1]*(Nx*Ny)/(px*py)*px + source_coords[0]*Nx/px], 1, blk_vector, p, p, comm_cart, MPI_STATUS_IGNORE);
    }
  }
